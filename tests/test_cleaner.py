...
def test_skips_symlinks(sample_file_tree):
    ...
    cleaner = HighThroughputDirCleaner(
        ...
        max_age_hours=36,
        dry_run=False # For this test we mustn't enable dry_run. We are testing that symlinks are skipped
        # by making sure it's not deleted. So enabling dry run will cause it to never delete anyway.
    )
